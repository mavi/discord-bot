...
exports.run = async (client, message, args) => {
  if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(`"\`Yönetici\`" olman gerek.`);
   ...
   let teyitçirolü = db.fetch(`teyitcirol_${message.guild.id}`)
   ...
   if (!role) return message.channel.send('Girilen rol bulunamadı.')
     ...
     db.delete(`teyitcirol_${message.guild.id}`)
     message.channel.send(`Rol başarıyla sıfırlandı.`)
     return
   }
 
   db.set(`teyitcirol_${message.guild.id}`, role.id)

   const embed = new Discord.RichEmbed()
    .setColor('RANDOM')
    .setTitle(`Teyitçi rolü başarıyla ${role.name} olarak tanımlandı.`)
   message.channel.send(embed);

};
  exports.help = {
    name: 'roltanımla-teyitçi'
  };